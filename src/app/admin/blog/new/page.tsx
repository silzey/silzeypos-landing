    ...
    const handleCreatePost = async (data: BlogFormData) => {
        ...
        if (data.imageUrl && !data.imageUrl.startsWith('https://')) {
            ...
            try {
                ...
                toast({
                    title: "Image Error",
                    description: `Could not find image "${data.imageUrl}" in storage folder "${folderName}". Please upload it first or check the filename.`,
                    variant: "destructive",
                });
                return; // Stop execution if the image is not found
            }
        }
